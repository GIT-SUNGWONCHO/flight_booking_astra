...
**로그인 방식(사용자 확정 2026-09-12):** 9232 예매·9233 계측 모두 기본은 **본인 네이버 연동**이다. 이전에는 9232만 와이프 스카이패스 아이디/비밀번호였고 네이버로 넘어가지 못하게 막혀 있었으나 사용자 지시로 해제했다. **따라서 9232 예매도 본인 계정으로 진행되며 와이프 마일리지를 쓰지 않는다.** 와이프 계정으로 되돌리려면 `.env`에 `KE_LOGIN_MODE=skypass`와 자격정보를 두면 되고, 그때는 실패해도 네이버로 전환하지 않는다. 네이버가 비밀번호를 물으면 프로그램은 거기서 멈추고 사람이 처리한다. **살아 있는 세션이 어느 계정인지는 화면만 보고 알 수 없으므로**, 로그인에 성공할 때마다 프로필 안에 `.ke-login-mode` 표시를 남기고 9232 준비 때 현재 정책과 대조한다. 표시가 없거나 다르면 **이전 계정으로 예매될 수 있으므로 준비를 중단**하고 로그아웃 후 재준비를 요구한다. 기존에 준비된 프로필(예: 와이프 세션이 살아 있는 Windows 9232)은 이 때문에 한 번 중단된다. 9233 계측은 언제나 본인 네이버이며 이 대조를 적용하지 않는다(계측 실패가 예매를 막지 않는다).

표시가 **다르면** 준비를 중단하고, 표시가 **아예 없으면**(사람이 직접 로그인한 경우) 중단 대신 사용자 확인을 요구한다. 네이버가 추가 인증을 물어 사람이 마무리한 세션은 프로그램이 계정을 알 수 없기 때문이다. 확인은 브라우저에 접속하지 않고 표시만 남긴다.

```bash
# 9232 에서 직접 네이버로 로그인한 뒤 한 번 실행한다
./.venv/bin/python dev/setup.py --port 9232 --confirm-login naver
```

이 명령은 로그인을 수행하지 않으며, 사용자가 그 계정으로 들어갔다고 **선언**하는 것이다. 잘못 선언하면 잘못된 계정으로 예매된다.
...
검증 지침은 [ke-verify](../../../.claude/skills/ke-verify/SKILL.md)를 따른다. 09시 실조건은 연속 2회 통과 전까지 `1/2`처럼 횟수를 명시한다. 교차 검토는 [ke-review](../../../.claude/skills/ke-review/SKILL.md)를 따르며, 코덱스 검토 통과는 위 T1~T3 어느 것도 대신하지 않는다. 신규 개방에 의존하는 코드는 미개방 → P 있음 → P 없음 전이 픽스처를 먼저 통과해야 한다.
...
실행 코드·`api_booking/` 변경을 커밋하기 전에 코덱스 교차 검토를 한 번 돌린다. 읽기 전용이며 주문·브라우저·시험을 실행하지 않는다. 지적은 자동 적용 대상이 아니고 채택 여부를 사람이 정한다. 상세 절차는 [ke-review](../../../.claude/skills/ke-review/SKILL.md)다.

```powershell
# Windows 기준. Mac 은 ./.venv/bin/python 으로 바꾼다
.\.venv\Scripts\python.exe dev\ke_review.py
.\.venv\Scripts\python.exe dev\ke_review.py --base codex/hybrid-booking
```

`codex` 실행 파일은 PATH → `KE_CODEX` → OS별 설치 경로 순으로 찾는다. 결과는 `dev-shots/reviews/<검토 ID>/`에 남고 `ke_review.json`의 `ok`가 `true`일 때만 `review.md`를 근거로 쓴다. 09시 준비·대기 중에는 실행하지 않는다.
