...
| ICN 도착 현대카드 선택→결제하기→현대카드 창 | **실조건 통과**(9/21 09:00 CDG→ICN 프레스티지, T0+28.2초 도착) |
| 무인 체인(작업 스케줄러 08:20 → 09:00 발사) | 9/19 콜드 스타트·실전에서 동작 확인 |
| 09:00 신규 개방 프레스티지 주문 확보 | **9/21 성공**(좌석 2석 중 두 번째). 좌석 1석이면 실패(9/19·9/20) |
...
- **같은 세션 요청은 겹치지 않는다.** 운임은 세션의 마지막 조회를 위치 번호(`recommendId`·`flightId`)로 참조한다.
  조회 응답 전 운임은 `ERT.3010`/`ERT.4002`, 조회 겹침 뒤 운임은 간격에 따라 `ERT.4002`(9/19 조사, [결과 §8](../results/2026-09-19.md)).
...
1. 09:00 경쟁: 주문 송신 **+2.6~2.7초**가 현재값, 이론 바닥 **+2.3초**(좌석 공개 −0.95초 + 조회 1.6초 + 운임 1.4초).
   경쟁자 주문 송신 추정은 **+0.8~1.8초**([9/21 §4](../results/2026-09-21.md)). 조회 생략·겹침·운임 생략이 모두 막혀
   **첫 좌석은 이길 수 없다**. 좌석 2석 이상이면 이긴다.
