...
| ICN 도착 현대카드 선택→결제하기→현대카드 창 | **리허설 통과**(9/20 01:30 FCO→ICN 일반석, 창 경로 수정 후 판정 `ready`) |
...
1. 09:00 경쟁: 주문 송신 +2.8~3.1초(조회 1.4~1.7초 + 운임 1.4초 직렬). 9/19 프레스티지는 +2.7~5.05초에 사라졌다 → 조회 생략·겹침은 불가(9/19 조사). 남은 수단은 선발사 값과 연결 예열.
