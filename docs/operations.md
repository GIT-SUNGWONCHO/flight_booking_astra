...
| 캡처에서 `inputTravellers` 미포착 | 같은 세션 두 번째 캡처면 사이트가 주문 요청을 다시 보내지 않는다. Chrome 재시작 뒤 첫 통과로 다시 준비(9/20) |
| 작업이 제시각에 안 뜸 | 같은 이름의 이전 실행 콘솔이 `pause`로 남아 있으면 새 기동이 무시된다(9/20). 창을 닫거나 `Stop-ScheduledTask` 뒤 재등록 |
