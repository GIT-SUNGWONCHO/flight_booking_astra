    ...
    try:
        ...
        while any(p.poll() is None for p in processes.values()):
            ...
            if check_at and now >= check_at:
                failures = checkpoint(processes, identity, folder, target, fire.isoformat())
                atomic_json(folder / "checkpoint.json", {"runId": identity, "ok": not failures, "failures": failures})
        ...
        for name, p in processes.items():
            ...
            results[name] = {"exitCode": code, "ok": code == 0 and valid and result.get("ok") is True,
